...
def config():
    ...
    # --- Handle Per-Camera Settings ---
    cameras_from_db = Camera.get_all()
    
    if request.method == 'POST':
        # ... (existing code for saving general config) ...

        # Process per-camera settings
        from app.utils.camera_processor import CameraManager
        manager = CameraManager.get_instance()
        
        form_cameras = request.form.to_dict(flat=False) # Get form data as dict of lists
        
        num_cameras_in_form = 0
        if 'cameras[0][id]' in request.form: # Check if camera data is present
            # Determine the number of cameras submitted in the form
            # This is a bit heuristic; assumes IDs are contiguous from 0
            i = 0
            while f'cameras[{i}][id]' in request.form:
                num_cameras_in_form += 1
                i += 1
        
        for i in range(num_cameras_in_form):
            camera_id = request.form.get(f'cameras[{i}][id]')
            if not camera_id:
                continue

            cam = Camera.get_by_id(camera_id)
            if not cam:
                flash(f"Camera with ID {camera_id} not found during update.", "warning")
                continue

            old_is_active = cam.is_active
            old_face_recognition_enabled = cam.face_recognition_enabled
            old_face_recognition_confidence = cam.face_recognition_confidence
            old_confidence_threshold = cam.confidence_threshold

            # Update camera fields from form
            cam.is_active = f'cameras[{i}][is_active]' in request.form
            cam.confidence_threshold = float(request.form.get(f'cameras[{i}][confidence_threshold]', cam.confidence_threshold))
            cam.face_recognition_enabled = f'cameras[{i}][face_recognition_enabled]' in request.form
            cam.face_recognition_confidence = float(request.form.get(f'cameras[{i}][face_recognition_confidence]', cam.face_recognition_confidence))
            
            cam.save()

            # Logic to restart camera processor or reload settings
            needs_restart = False
            if cam.is_active != old_is_active:
                needs_restart = True
            if cam.face_recognition_enabled != old_face_recognition_enabled:
                needs_restart = True
            if cam.face_recognition_confidence != old_face_recognition_confidence:
                needs_restart = True # If confidence changes, processor logic might need update
            if cam.confidence_threshold != old_confidence_threshold:
                needs_restart = True

            if needs_restart:
                if cam.is_active:
                    flash(f"Restarting camera {cam.name} due to settings change.", "info")
                    # manager.stop_camera(cam.id) # Ensure it's stopped before starting
                    # manager.start_camera(cam) # This will use new settings from DB
                    # Simpler: just tell manager to restart, it should handle fetching new config
                    if manager.get_camera_processor(cam.id): # If running
                         manager.stop_camera(cam.id)
                    manager.start_camera(cam) # Start will load new config including face settings
                else: # If camera was deactivated
                    flash(f"Stopping camera {cam.name} as it's now disabled.", "info")
                    manager.stop_camera(cam.id)
            
            # Reload persons if face recognition was just enabled or confidence changed
            # and camera is active
            if cam.is_active and (
                (cam.face_recognition_enabled and not old_face_recognition_enabled) or \
                (cam.face_recognition_enabled and cam.face_recognition_confidence != old_face_recognition_confidence)
            ):
                # The CameraProcessor's reload_known_persons will be triggered by start_camera
                # or if already running, we might need a specific call if start_camera doesn't re-init everything.
                # The CameraProcessor.__init__ now uses camera.face_recognition_enabled.
                # If the processor is already running, its internal state needs update.
                # A full restart (stop/start) is the most robust way to apply all settings.
                # If not restarting fully, then:
                processor = manager.get_camera_processor(cam.id)
                if processor and processor.is_running():
                     processor.camera = cam # Update processor's camera object
                     if cam.face_recognition_enabled:
                         processor.face_recognition_enabled = cam.face_recognition_enabled # Update this flag too
                         processor.reload_known_persons()
                     else:
                         processor.face_recognition_enabled = False
                         processor.known_persons = []


        # Reload cameras from DB to reflect changes in the UI immediately after POST
        cameras_from_db = Camera.get_all()


    return render_template('admin/config.html', title='System Configuration', 
                           config=config, system_stats=system_stats, cameras=cameras_from_db)