...
def get_camera_recordings_by_date(camera_id):
    ...
    try:
        ...
        # Process recordings one by one to ensure valid data
        for rec in recordings_cursor:
            try:
                recording = Recording(rec)
                # Quick validation of essential fields
                if recording.id and recording.timestamp and recording.file_path:
                    recordings.append(recording)
                else:
                    logger.warning(f"Skipping invalid recording record: {rec}")
            except Exception as e:
        ...
        # Process detections one by one to ensure valid data
        for det in detections_cursor:
            try:
                detection = Detection(det)
                # Quick validation
                if detection.id and detection.timestamp:
        ...
        if events_only and detections:
            ...
            for det in detections:
                if hasattr(det, 'recording_id') and det.recording_id:
        ...
        # Convert to dictionaries for JSON response, with error handling
        recordings_dict = []
        for r in recordings:
            try:
                rec_dict = r.to_dict()
                ...
                logger.error(f"Error converting recording to dict: {str(e)}")
                # Skip this recording
        ...
        for d in detections:
            try:
                ...
                logger.error(f"Error converting detection to dict: {str(e)}")
                # Skip this detection
...
@api_bp.route('/system/storage_drives')
@login_required
def get_storage_drives():
    """Get information about available storage drives"""
    import os
    import shutil
    import psutil
    ...
    try:
        ...
        # Format size values to human-readable format
        ...
        for partition in partitions:
            ...
            if partition.fstype == '' or not os.access(partition.mountpoint, os.R_OK | os.W_OK):
                continue
                
            try:
                usage = shutil.disk_usage(partition.mountpoint)
                
                # Only include drives with at least 100MB free space
                ...
                drives.append({
                    ...
                    'free': usage.free,
                    'available_formatted': format_size(usage.free),
        ...
        drives.sort(key=lambda d: d['free'], reverse=True)
        ...
        import traceback
        print(f"Error getting storage drives: {str(e)}")
...
@api_bp.route('/system/disk_info', methods=['GET'])
@login_required
@admin_required
def get_disk_info():
    """Get detailed information about a specific disk or path"""
    import os
    import subprocess
    import json
    import logging
    ...
    try:
        ...
        # Get more detailed information with lsblk
        device = None
        try:
            # Find device that path is mounted on
            result = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', path], 
                                   capture_output=True, text=True)
            if result.returncode == 0:
                device = result.stdout.strip()
                
                # If device path exists, get detailed info
                if device:
                    device_name = os.path.basename(device)
                    lsblk_result = subprocess.run(
                        ['lsblk', '-J', '-o', 'NAME,SIZE,FSTYPE,LABEL,MODEL,SERIAL', device_name], 
                        capture_output=True, text=True)
                    
                    if lsblk_result.returncode == 0:
                        lsblk_data = json.loads(lsblk_result.stdout)
                        device_info = lsblk_data.get('blockdevices', [{}])[0]
                    else:
        ...
        # Check if MongoDB is currently using this path
        mongodb_using_path = False
        try:
            # Check mongod process cmdline
            result = subprocess.run(['ps', '-o', 'args', '-C', 'mongod'], 
                                   capture_output=True, text=True)
            if result.returncode == 0:
                mongodb_using_path = path in result.stdout
                
            # Also check mongodb config
            if os.path.exists('/etc/mongodb.conf'):
                with open('/etc/mongodb.conf', 'r') as f:
                    mongodb_conf = f.read()
                    if f'dbPath: {path}' in mongodb_conf:
                        mongodb_using_path = True
        except Exception as e:
            logger.error(f"Error checking MongoDB usage: {e}")
        ...
        response = {
            ...
            'total_formatted': format_size(disk_usage.total),
            'used_formatted': format_size(disk_usage.used),
            'free_formatted': format_size(disk_usage.free),
            'used_percent': round((disk_usage.used / disk_usage.total) * 100 if disk_usage.total > 0 else 0),
            'device_info': device_info,
            'is_writable': os.access(path, os.W_OK),
            'mongodb_using_path': mongodb_using_path
        }
...
@api_bp.route('/system/format_drive', methods=['POST'])
@login_required
@admin_required
def format_drive():
    """Format a drive for database storage"""
    import subprocess
    import logging
    import os
    import json
    ...
    format_type = data.get('format_type', 'ext4')
    
    if not mount_point:
        return jsonify({
            'success': False,
            'error': 'Mount point parameter is required'
        }), 400
    
    # Validate format type
    valid_formats = ['ext4', 'xfs', 'btrfs']
    if format_type not in valid_formats:
        return jsonify({
            'success': False,
            'error': f'Unsupported filesystem type. Valid options are: {", ".join(valid_formats)}'
        }), 400
    ...
    try:
        ...
        if not os.path.exists(mount_point):
            return jsonify({
                'success': False,
                'error': f'Path does not exist: {mount_point}'
            }), 404
            
        # Check if MongoDB is using this path
        result = subprocess.run(['pgrep', '-f', f'mongod.*{mount_point}'], 
                              capture_output=True, text=True)
        if result.returncode == 0:
            return jsonify({
                'success': False,
                'error': f'MongoDB is currently using this path. Stop MongoDB service before formatting.'
            }), 400
        
        # Get the device name from mount point
        result = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', mount_point], 
                                capture_output=True, text=True)
        if result.returncode != 0:
            return jsonify({
                'success': False,
                'error': f'Could not determine device for mount point {mount_point}'
            }), 400
        ...
        if not device:
            return jsonify({
                'success': False,
                'error': f'No device found for mount point {mount_point}'
            }), 400
        
        # Unmount the device first
        logger.info(f"Unmounting device {device} from {mount_point}")
        unmount_result = subprocess.run(['umount', mount_point], 
                                      capture_output=True, text=True)
        if unmount_result.returncode != 0:
            return jsonify({
                'success': False,
                'error': f'Failed to unmount {mount_point}: {unmount_result.stderr}'
            }), 500
        
        # Create target directory if it doesn't exist
        os.makedirs(mount_point, exist_ok=True)
        
        # Format the device with selected filesystem
        logger.info(f"Formatting device {device} with {format_type}")
        
        format_cmd = None
        if format_type == 'ext4':
            format_cmd = ['mkfs.ext4', '-F', device]
        elif format_type == 'xfs':
            format_cmd = ['mkfs.xfs', '-f', device]
        elif format_type == 'btrfs':
            format_cmd = ['mkfs.btrfs', '-f', device]
        
        if not format_cmd:
            return jsonify({
                'success': False,
                'error': f'Unsupported filesystem type: {format_type}'
            }), 400
        ...
        if format_result.returncode != 0:
            return jsonify({
                'success': False,
                'error': f'Failed to format device: {format_result.stderr}'
            }), 500
        
        # Create mongodb directory
        mongodb_dir = os.path.join(mount_point, 'mongodb')
        os.makedirs(mongodb_dir, exist_ok=True)
        
        # Mount the device
        mount_options = data.get('mount_options', 'defaults,noatime')
        logger.info(f"Mounting device {device} to {mount_point} with options {mount_options}")
        
        mount_result = subprocess.run(['mount', '-o', mount_options, device, mount_point], 
                                    capture_output=True, text=True)
        if mount_result.returncode != 0:
            return jsonify({
                'success': False,
                'error': f'Failed to mount device: {mount_result.stderr}'
            }), 500
        
        # Set proper permissions for mongodb directory
        os.makedirs(mongodb_dir, exist_ok=True)
        subprocess.run(['chown', '-R', 'mongodb:mongodb', mongodb_dir], 
                     capture_output=True, text=True)
        
        return jsonify({
            'success': True,
            'message': f'Drive successfully formatted with {format_type} filesystem and mounted at {mount_point}'
        })
...
@api_bp.route('/system/mount_drive', methods=['POST'])
@login_required
@admin_required
def mount_drive():
    """Mount a drive for database storage"""
    import subprocess
    import logging
    import os
    ...
    try:
        ...
        if result.returncode == 0:
            return jsonify({
                'success': True,
                'message': f'Drive is already mounted at {mount_point}'
            })
        
        # Find device for this path from /etc/fstab or by device ID
        # First try fstab
        device = None
        ...
        # If not found in fstab, try finding by device ID or label
        if not device:
            logger.info(f"Mount point {mount_point} not found in fstab, checking block devices")
            result = subprocess.run(['blkid'], capture_output=True, text=True)
            if result.returncode == 0:
                lines = result.stdout.strip().split('\n')
                for line in lines:
                    if f'LABEL="{os.path.basename(mount_point)}"' in line:
        ...
        return jsonify({
            'success': True,
            'message': f'Drive successfully mounted at {mount_point}'
        })
...
@api_bp.route('/system/unmount_drive', methods=['POST'])
@login_required
@admin_required
def unmount_drive():
    """Unmount a drive"""
    ...
    try:
        ...
        if result.returncode == 0:
            return jsonify({
                'success': False,
                'error': f'MongoDB is currently using this path. Stop MongoDB service before unmounting.'
            }), 400
        ...
        if result.returncode != 0:
            return jsonify({
                'success': True,
                'message': f'Drive is not mounted at {mount_point}'
            })
        ...
        if unmount_result.returncode != 0:
            ...
            logger.warning(f"Regular unmount failed, trying force unmount")
            unmount_result = subprocess.run(['umount', '-f', mount_point], 
                                         capture_output=True, text=True)
            if unmount_result.returncode != 0:
                return jsonify({
                    'success': False,
        ...
        return jsonify({
            'success': True,
            'message': f'Drive successfully unmounted from {mount_point}'
        })
...
@api_bp.route('/system/restart_mongodb', methods=['POST'])
@login_required
@admin_required
def restart_mongodb_service():
    ...
    try:
        ...
        # Check if running as root/sudo (required to restart system services)
        is_root = os.geteuid() == 0
        
        if not is_root:
            # Try using sudo if not running as root
            restart_cmd = ['sudo', 'systemctl', 'restart', 'mongodb']
        else:
            restart_cmd = ['systemctl', 'restart', 'mongodb']
        
        restart_result = subprocess.run(restart_cmd, capture_output=True, text=True)
        
        if restart_result.returncode != 0:
            # Try alternative service name (mongod) if first attempt failed
            if not is_root:
                restart_cmd = ['sudo', 'systemctl', 'restart', 'mongod']
            else:
            ...
            if restart_result.returncode != 0:
                return jsonify({
                    'success': False,
                    'error': f'Failed to restart MongoDB service: {restart_result.stderr}'
                }), 500
        
        # Wait for MongoDB to come back online
        max_retries = 10
        ...
        for i in range(max_retries):
            ...
            status_cmd = ['systemctl', 'is-active', 'mongod']
            status_result = subprocess.run(status_cmd, capture_output=True, text=True)
            ...
            if i == max_retries - 1:
                logger.warning("MongoDB service did not come back online in the expected time")
        
        # Try to reconnect to the database to ensure it's working
        try:
            from app import db
            db.client.admin.command('ping')
            logger.info("Successfully reconnected to MongoDB after restart")
        except Exception as e:
            logger.error(f"Failed to reconnect to MongoDB: {e}")
            return jsonify({
                'success': False,
                'error': f'MongoDB restarted but connection failed: {str(e)}'
            }), 500
        
        return jsonify({
            'success': True,
            'message': 'MongoDB service restarted successfully'
        })
    
    except Exception as e:
        logger.error(f"Error restarting MongoDB service: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return jsonify({
            'success': False,
